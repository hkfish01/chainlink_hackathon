...
contract Whisky is ERC721A, 
    ...
    uint16[] public openTokenIdList;
    mapping (uint=>bool) public openStatus;
    mapping (uint=>uint) public tokenId2openTokenId;
    ...
    function _getOpenTokenId(uint _tokenId, uint _randomNum) private returns(uint){
        uint openTokenIdIndex = _randomNum % openTokenIdList.length;  //0--(openTokenIdList.length-1)
        tokenId2openTokenId[_tokenId] = openTokenIdList[openTokenIdIndex];
        _removeElement(openTokenIdIndex);
        return tokenId2openTokenId[_tokenId];
    }

    function _removeElement(uint index) private {
        require(index < openTokenIdList.length, "Invalid index");
        openTokenIdList[index] = openTokenIdList[openTokenIdList.length - 1];
        openTokenIdList.pop();
    }
    ...
    function tokenURI(uint _tokenId) public view virtual override returns (string memory) {
        ...
        if(!openStatus[_tokenId]){
            ...
            return bytes(uriOpenPrefix).length > 0
                    ? string(abi.encodePacked(uriOpenPrefix, tokenId2openTokenId[_tokenId].toString(), uriSuffix))
                    : '';
        }
